...
//------------------------------------------------------
    XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            await delay(1000 * 10)
          const CREDS = fs.readFileSync('./sessions/creds.json');
           try {
        const response = await axios.get("https://uploader.alpha-md.rf.gd/paste", {
            params: {
        ...
        console.error("Error uploading creds:", error);
            ...
            await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: pasteId });
            let sessionXeon = fs.readFileSync('./sessions/creds.json');
            await delay(1000 * 2) 
             const xeonses = await  XeonBotInc.sendMessage(XeonBotInc.user.id, { document: sessionXeon, mimetype: `application/json`, fileName: `creds.json` })
             await XeonBotInc.sendMessage(XeonBotInc.user.id, { text: `🙏 Thanks for using Alpha MD! Here is your creds & session ID 🔒` }, {quoted: xeonses});
              await delay(1000 * 2) 
              process.exit(0)
        }
        if (
